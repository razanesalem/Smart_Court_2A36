...
model->setHeaderData(3,Qt::Horizontal,QObject::tr("DATEINSCRIT"));
model->setHeaderData(4,Qt::Horizontal,QObject::tr("ETATAF"));
model->setHeaderData(5,Qt::Horizontal,QObject::tr("OBJECT"));
...
QSqlQueryModel*  Affaire:: triedate()
{

QSqlQueryModel* model =new QSqlQueryModel();

model->setQuery ("select * from affaires order by DATEINSCRIT");
...
//-----------------------------------recherche par type---------------------------------------
QSqlQueryModel *  Affaire::recherchertype(QString type)
 {
     QSqlQuery qry;
     qry.prepare("select * from affaires where TYPE=:type");
