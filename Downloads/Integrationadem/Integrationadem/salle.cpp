...
QSqlQueryModel * salle::researchsalle(QString recherche)
{
    QSqlQueryModel * model=new QSqlQueryModel();
        model->setQuery("SELECT * FROM SALLE WHERE NUM_SALLE LIKE '"+recherche+"%'");
        model->setHeaderData(0, Qt::Horizontal, QObject::tr("NUM_SALLE"));
        model->setHeaderData(1, Qt::Horizontal, QObject::tr("CAPACITE"));
...
QSqlQueryModel * salle:: view_tri_capacite()
{
    QSqlQueryModel * model= new QSqlQueryModel();
